...
`)
    ...
    expect(validationWorkspace).toContain("'apply', '--check', '--binary', '--whitespace=nowarn'")
    ...
    expect(validationWorkspace).toContain("'--untracked-files=no'")
    expect(validationWorkspace).not.toContain('WorkspaceManifest')
    expect(validationSandbox).not.toContain('scratchPaths')
