...
export function readProviderKeyFromFd(value: string): string {
  ...
  if (!Number.isSafeInteger(fd) || fd < 3 || fd > 1_024) throw new Error('Provider key fd is outside the allowed range.')
  ...
  try {
    ...
    try {
      ...
      // The read failure remains the authoritative error; never retry or log fd contents.
    }
  }
