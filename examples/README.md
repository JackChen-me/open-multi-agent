...
From the repository root, install dependencies once and run any of these deterministic examples:

```bash
npm install
npx tsx examples/patterns/event-driven-dag.ts
npx tsx examples/patterns/durable-approval.ts
npx tsx examples/patterns/eval-offline-regression.ts
```

- [`event-driven-dag`](patterns/event-driven-dag.ts) — see dependency-driven dispatch without waiting for unrelated work.
- [`durable-approval`](patterns/durable-approval.ts) — suspend a run, record a reviewer decision, and restore it in a fresh orchestrator.
- [`eval-offline-regression`](patterns/eval-offline-regression.ts) — run a deterministic offline evaluation and gate.
...
- **Imports** should resolve as `from '../../packages/core/src/index.js'` for scripts; full applications with their own `package.json` import the published `@open-multi-agent/core` package name instead.
